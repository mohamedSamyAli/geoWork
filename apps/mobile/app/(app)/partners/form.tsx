...
export default function PartnerFormScreen() {
  ...
  const { data: detailResult } = usePartnerDetail(isEdit ? id : undefined);
  ...
  const { control, handleSubmit, formState: { errors } } = useForm<CreatePartnerFormData>({
    ...
    <ScrollView style={[styles.container, { backgroundColor }]} contentContainerStyle={styles.content}>
      ...
      >
        {isPending ? (
          ...
          <Text style={styles.submitText}>{isEdit ? 'Update Partner' : 'Create Partner'}</Text>
        )}
      </Pressable>
    </ScrollView>
  );
